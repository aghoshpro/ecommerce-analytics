...
End-to-end data analytics project that ingests, cleans, transforms, and visualizes **96,457 delivered orders** from the Brazilian E-Commerce Public Dataset by Olist (the largest department store in Brazilian marketplaces.), covering the full data lifecycle from raw data ingestion to interactive BI dashboards through production-style medallion architecture (Bronze 🥉 → Silver 🥈 → Gold 🥇 layers) 

**The project answers four core business questions**: 

- Which product categories drive the most value? 
- Where is revenue growing? 
- Where are delivery operations failing? 
- What is the measurable cost of late delivery on customer satisfaction?

**Quick Result**

- [Dashboard Visualization](#results-and-dashboard-visualization)
- **[Interactive Tableau Dashboard](https://public.tableau.com/app/profile/arka.g1826/viz/OlistE-CommerceAnalytics_17873571676570/Dashboard1)**


<!-- [![CI Pipeline](https://github.com/aghoshpro/ecommerce-analytics/actions/workflows/ci.yml/badge.svg)](https://github.com/aghoshpro/ecommerce-analytics/actions/workflows/ci.yml)
...
[![Tableau](https://img.shields.io/badge/Tableau-Live%20Dashboard-E97627?style=flat&logo=tableau&logoColor=white)](https://public.tableau.com/app/profile/aghoshpro/viz/OlistE-CommerceAnalytics_17776790877880/Dashboard1) -->
...
- [Results and Dashboard Visualization](#results-and-dashboard-visualization)
...
- Raw CSV ingestion to AWS S3, PostgreSQL database with a Bronze/Silver architecture, 
- Python ETL automation with professional missing value handling, data quality validation, SQL-based business analysis across 8 dimensions, 
-Static visualization generation,
- Interactive BI dashboard delivery across three platforms (Tableau, Power BI, and Streamlit),
...
-  Place all 9 CSV files inside `data/raw/`

> The raw CSV files are excluded from Git via `.gitignore` due to their size. They must be downloaded manually before running the pipeline (before step 1).
...
```bash
git clone https://github.com/aghoshpro/ecommerce-analytics.git
...
python scripts_py/upload_to_s3.py

# Load all 9 raw tables into PostgreSQL
python scripts_py/load_to_postgres.py

# Clean all tables and produce 6 clean tables (Silver layer)
python scripts_py/clean_data.py

# Run 18 data quality validation checks
python scripts_py/validate_data.py

# Generate 7 visualization PNGs
python scripts_py/generate_visualizations.py

# Export 3 dashboard-ready CSVs
python scripts_py/export_dashboard_data.py
...
Produces 6 clean tables from the 9 raw tables using three distinct missing value strategies depending on context. Orders are filtered to delivered status only and enriched with two engineered columns: `delivery_days` (integer difference between purchase and delivery timestamps) and `is_late` (boolean flag comparing actual delivery against the estimated date). Products have the two column name typos corrected (`lenght` → `length`) and 610 null category names filled with `'unknown'` rather than dropped. Order items are filtered to delivered orders only. Reviews retain null comment text as meaningful absence but drop null scores. Location is deduplicated to one coordinate per zip code prefix. Payments remove three zero-value non-voucher rows as data entry errors.
...
Joins all 6 clean tables into a single flat CSV (`dashboard_data.csv`) with 21 columns covering order metadata, customer geography, product category, pricing, review score, payment method, and location coordinates. Also produces two pre-aggregated summary tables — `monthly_summary.csv` and `state_summary.csv` — which the Streamlit app and Power BI dashboard read directly for fast loading without re-joining at runtime.
...
Missing values and data quality decisions made during the pipeline, each with an explicit reason:
...
## Results and Dashboard Visualization
...
> Finally, processed files are put inside the 'data/processed'
...
Seven static charts generated by `scripts_py/generate_visualizations.py` and saved as high-resolution PNGs to `docs/visualizations/`. All are also displayed inside the Streamlit app.
...
- **[Interactive Tableau Dashboard](https://public.tableau.com/app/profile/arka.g1826/viz/OlistE-CommerceAnalytics_17873571676570/Dashboard1)**. The workbook file is also saved at `dashboard_tableau/olist_dashboard.twbx`.
...
Analytical dashboard built on the pipeline's processed CSV outputs. The `.pbix` file is available at `dashboard_powerbi/olist_dashboard.pbix` and can be opened directly in Power BI Desktop.
...
Streamlit interactive web application that presents all visualizations alongside KPI metrics and key findings. The app reads from pre-processed CSV files and displays pre-rendered PNG charts for fast loading.
